...
const AdminLootPanel = ({ dkpEnabled, refreshData }) => {
  ...
  const fetchAddedItems = async () => {
    try {
      ...
      const response = await axiosInstance.get(`/api/guild-storage/items?guildId=${guildId}&_t=${timestamp}`);
  ...
  const handleUpdate = async (id, field, value) => {
    try {
      ...
      // Convert timerDuration to number explicitly
      let updatedValue = value;
      if (field === 'timer_duration') {
  ...
  const handleAddItem = async () => {
    try {
      ...
      // Ensure timerDuration is a number and one of the valid options
      ...
      if (!validDurations.includes(timerDuration)) {
        setNotification({
          open: true,
          message: 'Invalid timer duration. Using default of 24 hours.',
          severity: 'warning'
        });
      }
      
      // Log what we're sending to help debug
      console.log('Adding item to storage with explicit timer duration:', {
        item_id: newItem.id,
        quantity: newItem.quantity,
        dkp_cost: newItem.dkpCost,
        trait: newItem.selectedTrait,
        timerDuration, // Explicitly log timer duration
        guildId
      });
      ...
      const response = await axiosInstance.post('/api/guild-storage', {
        ...
        dkp_cost: newItem.dkpCost,
        trait: newItem.selectedTrait,
        timerDuration, // Send as a number
        guildId
      });
      
      console.log('Add item response with timer_duration:', response.data);
      
      // Show success message with timer info
      setNotification({
        open: true,
        message: `Item added successfully with ${formatTimerDuration(timerDuration)} timer.`,
        severity: 'success'
      });
    ...
    try {
      ...
      try {
        ...
        if (alreadyRequested) {
          setNotification({
            open: true,
            message: "You already have a pending request for this item. Please check your requests tab.",
            severity: 'warning'
          });
        ...
        {newItem.id && (
          <Box sx={{ mb: 2, p: 2, bgcolor: 'rgba(144, 202, 249, 0.1)', borderRadius: 1 }}>
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              ...
              {/* Timer Duration Selection - NEW - Highlight with different background when non-default */}
              <Grid item xs={12} md={3}>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    label="Roll Timer"
                    onChange={(e) => {
                      console.log('Setting timer duration to:', e.target.value);
                      setNewItem({ ...newItem, timerDuration: Number(e.target.value) });
                    }}
                    sx={{
                      '& .MuiOutlinedInput-root': {
                        background: 'rgba(30, 30, 30, 0.4)',
                      ...
                      bgcolor: newItem.timerDuration !== 1440 ? 'rgba(255, 193, 7, 0.2)' : 'transparent'
                    }}
                  >
          ...
          <TableBody>
            {addedItems.map((item) => (
              <TableRow key={item.id} sx={{
                ...
                {/* Roll timer cell - NEW - with specific highlighting for non-default values */}
                <TableCell>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <AccessTimeIcon 
                      sx={{ 
                        color: item.timer_duration !== 1440 ? 'rgba(255, 193, 7, 0.8)' : 'rgba(144, 202, 249, 0.8)' 
                      }} 
                    />
                    <FormControl sx={{ minWidth: 120 }}>
                      <Select
                        value={Number(item.timer_duration) || 1440}
                        onChange={(e) => handleUpdate(item.id, 'timer_duration', Number(e.target.value))}
                        size="small"
                        sx={{ 
                          '.MuiSelect-select': { py: 0.5 },
                          bgcolor: item.timer_duration !== 1440 ? 'rgba(255, 193, 7, 0.1)' : 'rgba(30, 30, 30, 0.4)'
                        }}
                      >
                        {timerOptions.map(option => (
                          <MenuItem key={option.value} value={option.value}>
